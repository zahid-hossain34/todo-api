...
// Todo Interface (using JSDoc for type safety)
/**
...
const validateTodoBody = (body, isPartial = false) => {
  ...
  if (!isPartial) {
    ...
    if (
      ...
      errors.push(
        "Description is required and must be a string with at least 5 characters"
      );
    }
  } else {
    if (
      ...
      errors.push(
        "Title must be a string with at least 3 characters if provided"
      );
    }
    if (
      ...
      errors.push(
        "Description must be a string with at least 5 characters if provided"
      );
    }
  }
  ...
  try {
    ...
    if (!url.startsWith("/todos")) {
      return sendResponse(
        ...
        404,
        null,
        "Route not found. Use /todos or /todos/:id"
      );
    }
    ...
    if (urlParts.length === 3 && urlParts[1] === "todos") {
      ...
      if (todoIndex === -1) {
        return sendResponse(
          ...
          404,
          null,
          `Todo with ID ${parsedId} not found`
        );
      }
      ...
      switch (method) {
        ...
        case "PUT":
          ...
          const putErrors = validateTodoBody(putBody, true); // Partial update allowed
          ...
          todos[todoIndex] = {
            ...
            ...(putBody.description && {
              description: putBody.description.trim(),
            }),
            ...(putBody.completed !== undefined && {
              completed: putBody.completed,
            }),
        ...
        case "PATCH": // For toggling completion status
          const patchBody = await parseBody(req);
          if (
            patchBody.completed === undefined ||
            typeof patchBody.completed !== "boolean"
          ) {
            return sendResponse(
              res,
              400,
          ...
          sendResponse(
            ...
            200,
            todos[todoIndex],
            "Todo completion status updated"
          );
          return;
